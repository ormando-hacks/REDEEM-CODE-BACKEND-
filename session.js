...
async function start(phoneNumber) {
  ...
  sock.ev.on("connection.update", update => {
    ...
    if (connection === "close") {
      ...
      // Logged unconditionally (not through the pino logger, which defaults
      // to silent) so the real reason shows up in Render's log output.
      console.error("[session] connection closed:", JSON.stringify(state.lastError), lastDisconnect?.error?.stack || "");
